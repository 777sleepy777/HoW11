...
async def create_email(body: EmailModel, db: Session = Depends(get_db)):
    return await repository_emails.create_email(body, db)
...
async def update_email(body: EmailModel, email_id: int, db: Session = Depends(get_db)):
    email = await repository_emails.update_email(email_id, body, db)
    if email is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Email not found")
    return email
...
async def remove_email(email_id: int, db: Session = Depends(get_db)):
    email = await repository_emails.remove_email(email_id, db)
    if email is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Email not found")
    return email